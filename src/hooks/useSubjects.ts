...
export function useSubjects() {
  ...
  const addSubject = useCallback((data: Omit<Subject, "subject_id">) => {
    setSubjects((prev) => {
      ...
      const newId = maxId + 1;
      const next = [...prev, { ...data, subject_id: newId }];
      save(next);
      logActivity({ action: "created", entity: "Subject", entityId: String(newId), details: `Added subject ${data.subject_name}`, user: "admin", role: "admin" });
      return next;
    });
  }, []);
  ...
  const updateSubject = useCallback((updated: Subject) => {
    setSubjects((prev) => {
      ...
      save(next);
      logActivity({ action: "updated", entity: "Subject", entityId: String(updated.subject_id), details: `Updated subject ${updated.subject_name}`, user: "admin", role: "admin" });
      return next;
    });
  }, []);
  ...
  const removeSubject = useCallback((id: number) => {
    setSubjects((prev) => {
      const subject = prev.find((s) => s.subject_id === id);
      const next = prev.filter((s) => s.subject_id !== id);
      save(next);
      logActivity({ action: "deleted", entity: "Subject", entityId: String(id), details: `Removed subject ${subject?.subject_name || id}`, user: "admin", role: "admin" });
      return next;
    });
  }, []);
